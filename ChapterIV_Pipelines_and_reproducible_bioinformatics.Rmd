...
A) Execute `snakemake` to quantify all the samples with featureCounts and using 6 cores (hint: using a similar call than the fist command of section 5.1 - read the `snakemake --help`). 
B) Look inside `Snakefile`code to see where the final output of featureCount will be stored and compare it with the output we previously had using featureCounts.  
